...
def make_time_feats(ts):
    return [ts.hour/23.0, ts.month/12.0]
...
minority_idx = train_idx[y_train == minority_class]
majority_idx = train_idx[y_train == majority_class]
...
# ── 5) MODEL DEFINITION ────────────────────────────────────────────────────────
hist_in = Input((HIST_LEN, len(dyn_cols)), name="hist")         # [B, T, D_dyn]
stat_in = Input((X_stat.shape[1],), name="stat")                # [B, D_static]
time_in = Input((2,), name="time")                              # [B, 1]
sid_in  = Input((), dtype=tf.int32, name="station")             # [B]
...
# --- LSTM over history ---
x_hist = LSTM(64, return_sequences=False)(hist_in)              # [B, 64]

# --- Concatenate all inputs ---
x = Concatenate()([x_hist, stat_in, time_in, emb])              # [B, ?]

# --- Project to KAN input space ---
x = Dense(KAN_HID, activation="relu")(x)
x = Dropout(0.2)(x)
...
# --- Model compile ---
model = Model([hist_in, stat_in, time_in, sid_in], out)
model.compile(optimizer=Adam(LR), loss="binary_crossentropy", metrics=["accuracy", "AUC"])
...
# ── 7) SAVE MODEL ───────────────────────────────────────────────────────────
model.save("../model_testing/logistic_6_lstm.keras")